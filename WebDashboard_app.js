    ...
    const rfidModule = (function () {
      ...
      function render() {
        ...
        document.querySelectorAll('.stayCell[data-live="0"]').forEach(td => {
          const uid = String(td.dataset.uid || '').trim();
          const start = Number(td.dataset.start || 0);
      ...
      function handleRFIDMessage(payload) {
        try {
          ...
          if (!it) return; // cho phép row không có UID (đọc từ Google Sheet)
      ...
      const GAS_EXEC_URL = 'https://script.google.com/macros/s/AKfycbxsy64_iWlZfNUdVyCrZBeaoq_LmOEOcOL6ANTZAQkVovwUM0jz0-lTghSBcCpjlg8g/exec';
      const GAS_SHEET_NAME = 'Tất cả';
      const GAS_LIMIT = 10000;
      const GAS_POLL_MS = 1500;

      let _gasLastSig = '';

      function _gasRoomKeyFromText(phongText) {
        const s = normNoAccent(String(phongText || '')).toLowerCase().trim();
        if (!s) return 'unknown';

        // chấp nhận nhập dạng số: 1 / 2 / 3
        if (s === '1') return 'room1';
        if (s === '2') return 'room2';
        if (s === '3') return 'room3';

        // match nhiều kiểu ghi: phong/kho/room + số
        const is1 = /\b(room|phong|kho)\s*0*1\b/.test(s) || /\b(phong\s*kho|kho\s*lanh|phong\s*lanh)\s*0*1\b/.test(s) || /\bp0*1\b/.test(s);
        const is2 = /\b(room|phong|kho)\s*0*2\b/.test(s) || /\b(phong\s*kho|kho\s*lanh|phong\s*lanh)\s*0*2\b/.test(s) || /\bp0*2\b/.test(s);
        const is3 = /\b(room|phong|kho)\s*0*3\b/.test(s) || /\b(phong\s*kho|kho\s*lanh|phong\s*lanh)\s*0*3\b/.test(s) || /\bp0*3\b/.test(s);

        if (is1) return 'room1';
        if (is2) return 'room2';
        if (is3) return 'room3';

        // fallback: nếu có số 1/2/3 đứng riêng
        if (/\b1\b/.test(s)) return 'room1';
        if (/\b2\b/.test(s)) return 'room2';
        if (/\b3\b/.test(s)) return 'room3';

        return 'unknown';
      }

      function _ddmmyyyy_to_isoKey(ddmmyyyy) {
        const m = /^\s*(\d{1,2})\/(\d{1,2})\/(\d{4})\s*$/.exec(String(ddmmyyyy || ''));
      ...
      function normalizeRowFromSheet(rowObj) {
        if (!rowObj || typeof rowObj !== 'object') return null;

        const name = rowObj['Tên hàng hóa'] || rowObj['Ten hang hoa'] || rowObj['Tên hàng hoá'] || '';
        const phong = rowObj['Phòng kho'] || rowObj['Phong kho'] || rowObj['Phòng'] || rowObj['Phong'] || rowObj['Room'] || rowObj['room'] || '';
        const io = rowObj['Trạng thái nhập xuất'] || rowObj['Trang thai nhap xuat'] || rowObj['Trạng thái nhập/xuất'] || '';
        const dateVN = rowObj['Ngày/tháng/năm'] || rowObj['Ngay/thang/nam'] || '';
        const timeVN = rowObj['Thời gian'] || rowObj['Thoi gian'] || '';

        const nsx = rowObj['Ngày sản xuất'] || rowObj['Ngay san xuat'] || '';
        const hsd = rowObj['Hạn sử dụng'] || rowObj['Han su dung'] || '';
        let uid = rowObj['UID'] || rowObj['uid'] || rowObj['Mã thẻ'] || rowObj['Ma the'] || '';
        uid = String(uid || '').trim();
        if (!uid) {
          // Nếu sheet không có UID/Mã thẻ: tạo UID giả để tính "Thời gian đã ở trong kho" (ghép IN/OUT)
          const keyParts = [name, phong, nsx, hsd]
            .map(v => normNoAccent(String(v || '')).trim())
            .filter(Boolean);
          uid = keyParts.join(' | ').toUpperCase();
        }

        const dayKey = _ddmmyyyy_to_isoKey(dateVN);
        const timeStr = String(timeVN || '').trim().slice(0, 8);
        const tsMillis = Date.parse(`${dayKey}T${timeStr || '00:00:00'}`);

        const ex = computeExpireInfo(hsd);
        const rKey = _gasRoomKeyFromText(phong);

        // Nhiệt độ bảo quản: lấy theo phòng (bạn đang hiển thị như hình)
        const trange = (rKey === 'room1') ? '6–11°C' : (rKey === 'room2') ? '12–17°C' : (rKey === 'room3') ? '18–26°C' : '';

        return {
          tsMillis: Number.isFinite(tsMillis) ? tsMillis : Date.now(),
          _frozenStaySec: null,
          device_id: 'sheet',
          uid: String(uid || '').toUpperCase(),
          name: normalizeItemName(name),
          action: String(io || '').toUpperCase(),
          nsx: String(nsx || ''),
          hsd: String(hsd || ''),
          expire: ex.text,
          expireColor: ex.color,
      ...
      function _jsonp(url) {
        return new Promise((resolve, reject) => {
          const cb = '__gas_cb_' + Math.random().toString(16).slice(2);
      ...
      async function _fetchGasJson() {
        const base = GAS_EXEC_URL;
        if (!base || base.includes('PASTE_')) return null;
        const url = `${base}?sts=json&sheet=${encodeURIComponent(GAS_SHEET_NAME)}&limit=${GAS_LIMIT}`;

        // 1) try fetch (nếu cùng origin / đã bật CORS)
        try {
          const r = await fetch(url, { cache: 'no-store' });
          if (r.ok) return await r.json();
        } catch (e) { /* có thể do CORS */ }

        // 2) fallback JSONP (cần Apps Script hỗ trợ tham số callback)
        try {
          return await _jsonp(url);
        } catch (e2) { return null; }
      }

      async function _pollGasOnce() {
        const j = await _fetchGasJson();
        if (!j || !j.ok || !Array.isArray(j.data)) return;

        const sig = String(j.rows) + '|' + JSON.stringify(j.data[0] || {});
        if (sig === _gasLastSig) return;
        _gasLastSig = sig;

        const newRows = j.data.map(normalizeRowFromSheet).filter(Boolean);
        rows = newRows.slice(0, MAX_KEEP);

        render();
        saveRows();
      }

      // Chỉ poll khi đang ở tab "Quản lý hàng hóa" (rfid)
      setInterval(() => {
        const active = localStorage.getItem('coldroom.activeTab') || 'rfid';
        if (active !== 'rfid') return;
        _pollGasOnce();
      }, GAS_POLL_MS);
      _pollGasOnce();
    ...
    const roomMod = (function () {
      ...
      function applyAlarmWebUI(room) {
        ...
        if (!isAlarmWebEnabled(room) && room === currentRoom) {
          ...
          alarmLED.classList.add('hidden');
          alarmLED.classList.remove('red', 'pulse');
          alarmText.textContent = 'BÁO ĐỘNG NHIỆT ĐỘ: —';
        }
      }
      ...
      function updateAlarmBadge(room, val01, dir) {
        ...
        if (room === currentRoom) {
          ...
          alarmText.textContent = on ? ('BÁO ĐỘNG NHIỆT ĐỘ: ' + dirTxt) : 'BÁO ĐỘNG NHIỆT ĐỘ: —';
