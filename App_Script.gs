function doGet(e) {
  ...
  // ======== XUẤT JSON (EXPORT) ========
  // Ví dụ gọi: .../exec?sts=json&sheet=Tat%20ca
  //           .../exec?sts=json&sheet=Phong%20lanh%201
  //           .../exec?sts=json              -> mặc định "Tất cả"
  if (sts_val === 'json') {
    // tham số chọn sheet (tuỳ chọn)
    var sheetParam = '';
    if (e.parameter.sheet) sheetParam = stripQuotes(e.parameter.sheet);

    // Mặc định xuất sheet "Tất cả"
    var targetSheet = sheetMain;

    // Cho phép chọn theo từ khoá (loose)
    if (sheetParam) {
      var k = vnKey_(sheetParam);

      if (k === 'tat ca' || k === 'all') targetSheet = sheetMain;
      else if (k.indexOf('phong lanh 1') !== -1 || k === 'room1') targetSheet = sheetTPS;
      else if (k.indexOf('phong lanh 2') !== -1 || k === 'room2') targetSheet = sheetRCQ;
      else if (k.indexOf('phong lanh 3') !== -1 || k === 'room3') targetSheet = sheetCLK;
      else if (k.indexOf('an toan') !== -1 || k === 'safe') targetSheet = sheetSafe;
      else if (k.indexOf('can luu y') !== -1 || k === 'watch') targetSheet = sheetWatch;
      else if (k.indexOf('sap/da het han') !== -1 || k.indexOf('het han') !== -1 || k === 'danger') targetSheet = sheetDanger;
      else {
        // thử tìm trực tiếp theo tên nhập
        var trySheet = findSheetByNameLoose(ss, sheetParam);
        if (trySheet) targetSheet = trySheet;
      }
    }
    ...
    // Tham số giới hạn dòng (tuỳ chọn) để không trả quá nhiều
    var limit = 0;
    if (e.parameter.limit) limit = parseInt(stripQuotes(e.parameter.limit), 10) || 0; // 0 = không limit

    var json = exportSheetToJson_(targetSheet, limit);
    return ContentService
 ...
 return ContentService.createTextOutput('Only sts=write or sts=json is supported');
}
...
function exportSheetToJson_(sheet, limit) {
  var lastRow = sheet.getLastRow();
  var lastCol = sheet.getLastColumn();

  if (lastRow < 1 || lastCol < 1) {
    return { ok: true, sheet: sheet.getName(), rows: 0, data: [] };
  }
  ...
  var numRows = lastRow - 1;
  if (limit && limit > 0 && limit < numRows) numRows = limit;

  var values = sheet.getRange(2, 1, numRows, lastCol).getDisplayValues();
  ...
  return { ok: true, sheet: sheet.getName(), rows: out.length, data: out };
}
