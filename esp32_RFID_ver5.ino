...
#include <WiFiManager.h>  // <<-- dùng WiFiManager
...
#include <Preferences.h>  // NEW: lưu trạng thái vào flash

// ===== MQTT (HiveMQ Cloud TLS 8883) =====
constexpr const char* mqtt_server = "dcede8aa2beb496b980ed91f6804346e.s1.eu.hivemq.cloud";
constexpr int mqtt_port_tls = 8883;
constexpr const char* mqtt_user = "Huy-DTVT17B";
constexpr const char* mqtt_pass = "GiaHuy2008@";

// ===== Device & Topics =====
constexpr const char* DEVICE_ID = "esp32-RFID";
String topicRFID = String("coldroom/") + DEVICE_ID + "/rfid";
String topicCmd = String("coldroom/") + DEVICE_ID + "/cmd/mode";
String topicState = String("coldroom/") + DEVICE_ID + "/state";
String topicAlert = String("coldroom/") + DEVICE_ID + "/alert";
...
constexpr const char* gsScriptUrl =
  "https://script.google.com/macros/s/AKfycbzxQ9c75bwgTEW03NEVAhVCnV3MavQpFpJMQ_GnWyY-VibDzsDVPFtDmS6LSegfQUg/exec";
...
// ===== OLED I2C =====
// OLED 0.96" 128x64 trên bus Wire (I2C0): Date/Time/Trang thai
// OLED 0.91" 128x32 trên bus I2C_1 (I2C1): màn hình chờ / nội dung quét
#define OLED_RESET -1
...
// Gán tên theo 3 nhóm:
// - Thực phẩm sống: Thịt heo, Thịt bò, Thịt gà, Tôm, Cá
// - Rau củ quả: Rau, Củ, Trái cây
// - Các loại khác: Trứng, Sữa, Nước ngọt
...
// Map UID -> Room + Temperature range (based on your comment groups)
...
// URL encode để gửi tiếng Việt / dấu / khoảng trắng
...
void publishModeState(const char* status = "online") {
  String st = "{";
  st += "\"device_id\":\"" + String(DEVICE_ID) + "\",";
  st += "\"status\":\"" + String(status) + "\",";
  st += "\"mode\":\"" + String((currentMode == MODE_IN) ? "IN" : "OUT") + "\"";
  st += "}";
  mqttClient.publish(topicState.c_str(), st.c_str(), true);
}
...
void oledShowIdle() {
  ...
  if (oledMainOk) {
    ...
    displayMain.setTextSize(2);  // dòng trạng thái to hơn
...
void oledShowNameTimeDate(const String& name, const String& timeStr, const String& dateStr) {
  ...
  // ==== OLED 0.96" giống oledShowMessage ====
  if (oledMainOk) {
    displayMain.clearDisplay();
    displayMain.setTextColor(SSD1306_WHITE);
...
void setup_wifi_wm() {
  ...
  // Timeout cho cổng cấu hình: 300s (5 phút)
  wm.setConfigPortalTimeout(300);

  // Tên AP cấu hình, ví dụ: RFID-Setup-esp32-RFID
...
String makeClientId() {
  ...
  char cid[32];
  snprintf(cid, sizeof(cid), "esp32-%04X%04X", (uint16_t)(chipid >> 32), (uint16_t)chipid);
  return String(cid);
}
...
void mqttCallback(char* topic, byte* payload, unsigned int len) {
  ...
  if (t == topicCmd) {
    msg.toUpperCase();
    if (msg == "IN") currentMode = MODE_IN;
    else if (msg == "OUT") currentMode = MODE_OUT;
    else if (msg == "TOGGLE") toggleMode();
    else if (msg == "RESET") {
      resetAllStates();  // NEW: xóa cả RAM + NVS
      Serial.println("[STATE] Reset all item states to NEVER_SEEN");
    }
    publishModeState();
    resetAntiDuplicate();
    oledShowIdle();
...
time_t lastInEpoch[NUM_UID];  // thời điểm IN đầu tiên

// ===== NSX / HSD gán theo từng UID (đúng quy luật thời gian bảo quản) =====
// Thứ tự PHẢI KHỚP với knownUIDs[] (17 UID)
...
void publishEvent(const String& uid,
  ...
  // ===== NEW: temperature range for classification =====
  payload += "\"room\":\"" + String(room) + "\",";  // room1/room2/room3
  payload += "\"tmin\":" + String(tmin) + ",";
  payload += "\"tmax\":" + String(tmax);

  payload += "}";

  mqttClient.publish(topicRFID.c_str(), payload.c_str());
  Serial.println("MQTT EVENT: " + payload);
}


void publishAlert(const String& uid, const char* name, const char* level, const String& message, const String& ts) {
  String payload = "{";
  payload += "\"device_id\":\"" + String(DEVICE_ID) + "\",";
  payload += "\"uid\":\"" + uid + "\",";
  payload += "\"name\":\"" + String(name) + "\",";
  ...
  payload += "\"timestamp\":\"" + ts + "\"";
  payload += "}";
  mqttClient.publish(topicAlert.c_str(), payload.c_str());
  Serial.println("MQTT ALERT: " + payload);
}
...
void setup() {
  ...
  // Thay vì reset toàn bộ trạng thái mỗi lần khởi động,
  // ta chỉ reset RAM & cố gắng load lại từ NVS
  resetAllStatesRAM();
  loadStatesFromNVS();
...
void loop() {
  ...
  // Xử lý MQTT chỉ khi WiFi đang kết nối
  if (WiFi.status() == WL_CONNECTED) {
    ensureMQTT();
    mqttClient.loop();
  }
  ...
  if (pressStart && (ms - pressStart) >= LONG_PRESS_MS) {
    resetAllStates();  // NEW: reset cả RAM + NVS
    publishAlert("-", "System", "info", "RESET all states by long-press", nowStr());
    pressStart = 0;
  }
  ...
  if (st == NEVER_SEEN) {
    if (currentMode == MODE_OUT) {
      ...
      publishEvent(uid, name, "IN", ts, nsxArr[idx].c_str(), hsdArr[idx].c_str());
      enqueueGSheet("IN", "");
      states[idx] = INSIDE;
    ...
    if (currentMode == MODE_IN) {
      ...
      publishEvent(uid, name, "OUT", ts, nsxArr[idx].c_str(), hsdArr[idx].c_str());
