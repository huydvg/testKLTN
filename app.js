(function () {
      ...
      // ===== Cache nhiệt độ/độ ẩm mới nhất của 3 phòng =====
      const latestTH = {
      ...
      const roomMod = (function () {
        ...
        function applySysEnableUI(room) {
          ...
          // Nếu OFF: ẩn badge alarm + LED để tránh hiểu nhầm
          if (!enabled) {
        ...
        // Khi user bấm bật/tắt hệ thống phòng
        if (roomSysEnableEl) {
          roomSysEnableEl.addEventListener('change', () => {
            ...
            if (!en) {
              // “Chắc chắn tắt” thêm: yêu cầu peltier off + disable alarm phần cứng
              // (phòng trường hợp firmware bạn chưa xử lý sysEnable đủ triệt để)
              try {
        ...
        // Lưu tối đa 100000 bản ghi (có thể bị giới hạn bởi dung lượng LocalStorage của trình duyệt)
        const STORAGE_MAX = 100000;
        const VIEW_MAX_POINTS = 1000;     // gán cứng
        const VIEW_MS = 24 * 60 * 60 * 1000; // 24 giờ gán cứng

        // record compact: [ts, t, h, door(0/1), alarmTemp(0/1), alarmDir(-1/0/1), ev]
        // ev: 0=telemetry, 1=door_event, 2=alarm_on, 3=alarm_off(clear)
        ...
        function buildViewSeries(room) {
          ...
          // t/h series from telemetry only
          const tele = [];
          const doorEv = [];
          const alarmOnEv = [];
          const alarmOffEv = [];
          ...
          // downsample telemetry to max 1000 points
          let sampled = tele;
        ...
        const chart = new Chart(ctx, {
          ...
          options: {
            ...
            scales: {
              x: {
                ticks: {
                  display: false,              // ✅ Ẩn dòng thời gian (nhãn giờ) dưới biểu đồ
                  autoSkip: true,
                  maxRotation: 0,
                  color: getComputedStyle(document.documentElement).getPropertyValue('--muted')
                },
                grid: { color: 'rgba(148,163,184,.15)' }, // vẫn giữ lưới nếu bạn muốn
                border: { display: false }                 // (tuỳ chọn) ẩn đường trục X
              },
              y: {
        ...
        const LS_TH_TABLE_PREFIX = 'roomTH.table.v2.'; // per room
