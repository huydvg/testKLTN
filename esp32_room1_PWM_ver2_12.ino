/*
  ESP32 ColdRoom Firmware (FULL - copy/paste) ✅ KHÔNG DÙNG ledc*
  - WiFi ưu tiên: SAVED -> DEFAULT -> WiFiManager portal (AP pass 12345678)
  - MQTT TLS (HiveMQ Cloud) / retained state + telemetry
  - DHT22 + Kalman lọc
  - OLED SH1106 128x64
  - Door sensor interrupt
  - Peltier BTS7960: manual / auto / off
  - Alarm nhiệt độ (hysteresis) + buzzer nhịp 0.7s/0.3s
  - ✅ MASTER SYSTEM: khi Web tắt hệ thống => TẮT HOÀN TOÀN NGOẠI VI
      + BTS7960 EN=LOW
      + PWM=0 + khóa pin PWM về GPIO LOW (KHÔNG ledcDetachPin)
      + BUZZER OFF
      + detachInterrupt cửa
      + OLED OFF
      + (telemetry vẫn gửi để theo dõi)
*/
...
// Mục tiêu: khi SYS OFF, pin phải "đứng LOW" như GPIO thường.
// Không gọi ledcDetachPin, chỉ ép pin về OUTPUT LOW sau khi analogWrite(0).
void pwmHardOffPin(int pin) {
  analogWrite(pin, 0);     // dừng duty
  delay(1);                // cho nó kịp cập nhật (nhỏ thôi)
  pinMode(pin, OUTPUT);    // ép về GPIO
  digitalWrite(pin, LOW);  // giữ LOW cứng
}
...
void applySystemEnable(bool en) {
  ...
  if (!gSystemEnable) {
    // --- SYS OFF: tắt sạch ngoại vi ---
    gPeltierEnable = false;
    gPeltierMode = PELTIER_OFF;

    // PWM cứng LOW + tắt driver
    peltierPwmHardOff();
    gPeltierDuty = 0;
    ...
    // để pin về PWM mode lại bằng analogWrite(0)
...
void loop() {
  ...
  if (millis() - lastTeleMs > 1000) {
    ...
    // Serial Plotter: RAW \t KALMAN (không chữ)
    if (isfinite(gTempRaw) && isfinite(gTempKal)) Serial.printf("%.2f\t%.2f\n", gTempRaw, gTempKal);
    else if (isfinite(gTempRaw)) Serial.printf("%.2f\t\n", gTempRaw);
    else if (isfinite(gTempKal)) Serial.printf("\t%.2f\n", gTempKal);
    else Serial.println();
